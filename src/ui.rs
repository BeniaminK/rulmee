...
impl UI {
    pub fn new(ctx: UIContext) -> Self {
        ...
        let (selected_session_idx, custom_session, session_input) = match ctx.initial_session {
            Some(name) => {
                ...
                } else if include_defshell
                    && ctx
                        .users
                        .get(selected_user_idx)
                        .is_some_and(|u| u.shell == name)
                {
    ...
    pub fn check_hotkey(&self, key_code: KeyCode) -> Option<HotkeyAction> {
        ...
        if parse(&self.config.functions.poweroff) == Some(key_code) {
            Some(HotkeyAction::Poweroff)
        } else if parse(&self.config.functions.reboot) == Some(key_code) {
            Some(HotkeyAction::Reboot)
        } else if parse(&self.config.functions.fido) == Some(key_code) {
            Some(HotkeyAction::Fido)
        } else if parse(&self.config.functions.theme) == Some(key_code) {
    ...
    pub fn cursor_offset(&self) -> u16 {
        ...
        let selector = if self.show_selector(self.focused_field) {
    ...
    fn draw(&self, f: &mut Frame) {
        ...
        f.set_cursor_position(Position::new(
            chunks[fi].x + LABEL_W + GAP_W + self.cursor_offset(),
            chunks[fi].y,
        ));
    }
    ...
    pub fn layout(
        ...
        (
            login_area,
            Layout::vertical(field_constraints).split(inner).to_vec(),
            console_area,
        )
    }
    ...
    fn render_field(&self, buf: &mut Buffer, area: Rect, field: Field) {
        ...
        Paragraph::new(Span::styled(self.field_label(field), ls))
            .alignment(ratatui::layout::Alignment::Right)
            .render(la, buf);
        let value = self.field_display_text(field);
        let spans = if self.show_selector(field) {
            vec![
                Span::raw(&self.config.strings.opts_pre),
                Span::styled(value, vs),
                Span::raw(&self.config.strings.opts_post),
            ]
        } else {
...
impl Widget for &UI {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        if cfg.behavior.show_theme {
            ...
            let truncated =
                truncate_str(&display, bottom_row.width as usize, &cfg.strings.ellipsis);
            Paragraph::new(Span::styled(
                truncated,
...
mod tests {
    ...
    fn test_show_theme_rendering() {
        ...
        let bottom_text: String = (0..area.width)
            .filter_map(|x| buf.cell((x, area.height - 1)).map(|c| c.symbol()))
            .collect();
    ...
    fn test_configured_session_type_strings() {
        ...
        let sessions = vec![
            Session {
                name: "Sway".into(),
                exec: ExecType::Desktop(vec!["sway".into()]),
                session_type: SessionType::Wayland,
                desktop_names: None,
            },
            Session {
                name: "i3".into(),
                exec: ExecType::Desktop(vec!["i3".into()]),
                session_type: SessionType::Xorg,
                desktop_names: None,
            },
            Session {
                name: "bash".into(),
                exec: ExecType::Shell("bash".into()),
    ...
    fn test_auth_error_styling_and_clearing() {
        ...
        assert_eq!(
            ui.field_value_style(Field::Password),
